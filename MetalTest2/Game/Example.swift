...
final class Examples {
    ...
    func createP2(){
        ...
        guard let plane = Primitives.plane(renderer: renderer, width: 10, height: 10)else {
        ...
        var texture: Texture?
        if true {
            guard let metalTexture = Texture.loadMetalTexture(device: renderer.device, named: "dog") else {
                return
            }
        ...
        videoMaterial = Material.createBasic(renderer: renderer, texture0: texture)
        
        plane.material = videoMaterial
        
        videoNode = Node(mesh: plane)
        renderer.scene.root.addChild(videoNode)

    }
    
    func createPlaneScene(textured: Bool){
        renderer.scene.root.clearAllChildren()
        
        guard let plane = Primitives.plane(renderer: renderer, width: 0, height: 0)else {
            print("Failed to create the cuboid mesh")
            return
        }
        ...
        videoNode = Node(mesh: plane)
        videoNode.update = {[weak self] (time: Time, node: Node) in
//            node.orientation *= Quaternion(
//                angle: Math.toRadians(30.0) * Float(time.updateTime),
//                axis: [0.5, 1, -1]
//            )
            node.mesh?.material = self?.videoMaterial
        }
        ...
        let material2 = Material.createBasic(renderer: renderer, texture0: texture)
        
        cubeMesh.material = material2
        let node2 = Node(mesh: cubeMesh)
        node2.update = { (time: Time, node: Node) in
            node.orientation *= Quaternion(
                angle: Math.toRadians(30.0) * Float(time.updateTime),
                axis: [0.5, 1, -1]
            )
        }
        videoNode.addChild(node2)
        renderer.scene.root.addChild(videoNode)
    ...
    func updateVideoMaterial(newTexture : Texture){
        videoMaterial = Material.createBasic(renderer: renderer, texture0: newTexture)
        videoTexture = newTexture
    }
}

extension Examples : OperationChain{
   
    
    func newTextureAvailable(_ texture: Texture) {
        updateVideoMaterial(newTexture: texture)
    }
}
